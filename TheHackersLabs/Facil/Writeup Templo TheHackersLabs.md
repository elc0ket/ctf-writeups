...
Para entender cómo renombra el servidor los archivos, aprovechamos la LFI ya confirmada para filtrar el código fuente de `index.php` mediante el wrapper `php://filter`:
...
Decodificamos la respuesta en base64 para leer el código fuente:
...
Analizando el código se confirma que el servidor aplica `str_rot13()` sobre el nombre del archivo (sin extensión) antes de guardarlo. Con esto, en un decodificador online subimos el nombre de nuestro archivo para que lo codifique en ROT13, y así sabemos cómo se llamará nuestro archivo subido.
...
Descargamos el script que generará la imagen:
...
La pasamos a la máquina víctima sirviéndola por HTTP:
